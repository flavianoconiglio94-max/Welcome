...
RPC (`0002_booking_rpcs.sql`): `get_availability(restaurant_id, date, party_size)` genera slot da `opening_hours` e verifica disponibilità tavoli; `create_reservation(...)` auto-assegna il tavolo più piccolo che ha capienza libera (o lascia `table_id` null se non specificato/non trovato), aggiorna `guest_directory`; `cancel_reservation(reservation_id, cancellation_token)` autorizza via token (guest) o via sessione staff/admin.
...
- **Fase 1 (schema + RPC)** — ✅ COMPLETATA (codice locale): migrazioni `0001_multi_tenant_schema.sql` e `0002_booking_rpcs.sql` scritte e pronte, **ma non ancora applicate a un vero progetto Supabase** (bloccato dal limite di 2 progetti free sull'org precedente — da qui la necessità del nuovo account).
